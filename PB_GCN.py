...
class Model:
    ...
    def __init__(self, use_elmo, use_glove, use_extra_feature, encoding_size, pos_emb_size, ner_emb_size, pos_dict_size,
                 ner_dict_size, max_nodes=600, max_query_size=25, glove_dim=300, query_encoding_type='lstm'):
    ...
    def modelProcessing(self, query_length, adj, nodes_mask, bmask, nodes_elmo, query_elmo, nodes_glove, query_glove,
        ...
        logger.info("---------------------")
        logger.info("before processing GCN")
        logger.info("input nodes shape %s", nodes.shape)
        logger.info("query compress shape : %s", query_compress.shape)
        logger.info("---------------------")
        last_hop = nodes
        for _ in range(hops):
            last_hop = self.GCNLayer(adj, last_hop, nodes_mask,
                                     query_compress)  # last_hop=(batch_size, max_nodes, node_feature_dim)
    ...
    def featureLayer(self, query_length, nodes_elmo, query_elmo, nodes_glove, query_glove):
        ...
        with tf.variable_scope('feature_layer', reuse=tf.AUTO_REUSE):
            ...
            if self.use_elmo:
                # query_flat = tf.reshape(query_elmo, (-1, self.max_query_size, 3 * 1024))x
    ...
    def GCNLayer(self, adj, hidden_tensor, hidden_mask, query_compress):
        with tf.variable_scope('hop_layer', reuse=tf.AUTO_REUSE):
            ...
            update = tf.reduce_sum(tf.matmul(adjacency_tensor, hidden_tensors), 1) + tf.layers.dense(
                hidden_tensor, units=hidden_tensor.shape[-1])
            logger.info("update message's shape: %s", update.shape)
            update = update  * tf.expand_dims(hidden_mask, -1)
            logger.info("update message's shape[-1]: %s", update.shape[-1])

            """ query precessing start"""
            # partial dim = batch size x max nodes x max query
            expend_query = tf.tile(query_compress, multiples=[1, 24, 1])
            concatenated = tf.concat((expend_query, update), -1)    # (batch, 600, 1024)
            logger.info("concatenated shape :%s", concatenated)
            logger.info("output space dimension: %s", query_compress.shape[1])
            partial = tf.layers.dense(concatenated, units=query_compress.shape[1], activation=tf.nn.sigmoid) #  dim = 25
            logger.info("partial one's shape is %s", partial)
            partial * tf.expand_dims(hidden_mask, -1)
            partial = tf.nn.softmax(partial, -1) #  600 x 25
            new_query = tf.matmul(partial, query_compress)    # batch x maxnodes x dimension


            """query preprocessing end"""
            query_att = tf.layers.dense(tf.concat((new_query, update), -1), units=hidden_tensor.shape[-1],
                                        activation=tf.nn.sigmoid) * tf.expand_dims(hidden_mask, -1)

            # new update
            update = query_att * tf.nn.tanh(new_query) + (1 - query_att) * update

            att = tf.layers.dense(tf.concat((update, hidden_tensor), -1), units=hidden_tensor.shape[-1],
                                  activation=tf.nn.sigmoid) * tf.expand_dims(hidden_mask, -1)
            logger.info("attention shape: %s", att.shape)
...
class Optimizer:
    ...
    def __init__(self, model, use_elmo, use_glove, use_extra_feature, pos_dict_size, ner_dict_size, max_nodes=600,
        ...
        logger.info("tf.expand_dims(self.nodes_length, -1) shape is %s", tf.expand_dims(self.nodes_length, -1).shape)
        logger.info("nodes mask's shape: %s", self.nodes_mask.shape)
        logger.info("query_length shape : %s", self.query_length.shape)
        logger.info("query_length : %s", self.query_length)
        logger.info("adj shape : %s", self.adj.shape)
        logger.info("bmask shape : %s", self.bmask.shape)
        logger.info("nodes elmo shape :%s", self.nodes_elmo.shape)
        logger.info("nodes glove shape : %s", self.nodes_glove.shape)
        logger.info("query glove shape : %s", self.query_glove.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_elmo', type=str2bool, default=True,
                        help='Whether use the ELMo as a feature for each node')
    parser.add_argument('--use_glove', type=str2bool, default=True,
                        help='Whether use the GloVe as a feature for each node')
    ...
    parser.add_argument('--hop_num', type=int, default=4,
                        help='Hop num in Gated R-GCN layer, in other words the depth of GCN')
    ...
    if not evaluation_mode:
        logger.info('Loading preprocessed training data file %s',
                    '{}.preprocessed.pickle'.format(train_file_name_prefix))
        dataset = Dataset(train_file_name_prefix, use_elmo, use_glove, use_extra_feature, max_nodes=600,
                          max_query_size=25, max_candidates=80, max_candidates_len=10)
        logger.info('Loading preprocessed development data file %s',
                    '{}.preprocessed.pickle'.format(dev_file_name_prefix))
        dev_dataset = Dataset(dev_file_name_prefix, use_elmo, use_glove, use_extra_feature, max_nodes=600,
                              max_query_size=25, max_candidates=80, max_candidates_len=10)
    else:
        logger.info('Loading preprocessed evaluation data file %s',
                    '{}.preprocessed.pickle'.format(dev_file_name_prefix))
        dataset = Dataset(dev_file_name_prefix, use_elmo, use_glove, use_extra_feature, max_nodes=600,
                          max_query_size=25, max_candidates=80, max_candidates_len=10)
